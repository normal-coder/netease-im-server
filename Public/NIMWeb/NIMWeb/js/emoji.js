...
function buildEmoji(text) {
	...
	for (var j = 0, len = matches.length; j < len; ++j) {
		if(emoji[matches[j]]){
			text = text.replace(matches[j], '<img class="emoji" src="../../../Public/NIMWeb/NIMWeb/images/emoji/' + emoji[matches[j]].file + '" />');
		}		
	}
	return text;
}
...
CEmojiEngine.prototype.__renderChangeCol = function(){
	for(var i in this._emojiList){
		...
		for(var key in emojiList){
			...
			span.id = 'chn-emoji-'+i;	
			var img = new Image();
			img.src = "../../../Public/NIMWeb/NIMWeb/images/"+emojiList[key]['file'];
	...
	for(var i in this._pinupList){
		...
		span.id = 'chn-pinup-'+i;
		var img = new Image();
		img.src = "../../../Public/NIMWeb/NIMWeb/images/"+ pinupList[0];
		span.appendChild(img);
		this._changeColumn.appendChild(span);		
	}
}
